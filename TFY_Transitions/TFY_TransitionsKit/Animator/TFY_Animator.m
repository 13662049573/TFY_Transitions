...
typedef void(^AnimationCompletion)(BOOL flag);

@interface TFY_Animator ()<CAAnimationDelegate>
...
- (Direction)interactiveDirectionOfPop {
    if (self.type == AnimatorTypeTiltLeft || self.type == AnimatorTypeSlidingDrawer){
        return DirectionToLeft;
    }
    return DirectionToRight; // 滑动方向
}

- (CGFloat)percentOfFinishInteractiveTransition {
    if (self.type == AnimatorTypeTiltLeft || self.type == AnimatorTypeTiltRight){
        return 0.29f;
    }
    if (self.type == AnimatorTypeCircular || self.type == AnimatorTypeFlip){
        return 0.f;
    }
    if (self.type == AnimatorTypeSlidingDrawer){
        return 0.45;
    }
...
+ (instancetype)animatorWithType:(AnimatorType)type {
    TFY_Animator *anmator = [self new];
    anmator.type = type;
    if (type == AnimatorTypeFrame) {
        anmator.initialFrame = CGRectMake(transit_ScreenW * 0.5f, transit_ScreenH * 0.5f, 0.f, 0.f);
    }else if (type == AnimatorTypeFlip) {
        anmator.animationOptions = UIViewAnimationOptionTransitionFlipFromLeft;
    }else if (type == AnimatorTypeSlidingDrawer){
...
- (void)tiltTypeTransition:(id<UIViewControllerContextTransitioning>)transitionContext presenting:(BOOL)isPresenting {
    ...
    CGFloat W = animateView.bounds.size.width;
    CGFloat H = animateView.bounds.size.height;
    CGFloat angle = self.type == AnimatorTypeTiltLeft ? -M_PI_2 : M_PI_2;
...
- (void)rectScaleTypeTransition:(id<UIViewControllerContextTransitioning>)transitionContext presenting:(BOOL)isPresenting {
    ...
    if(_isOnlyShowRangeForRect) {
        bgView.backgroundColor = fromView.backgroundColor ? fromView.backgroundColor : transit_WhiteTintColor;;
    }else {
    ...
    [UIView animateWithDuration:[self transitionDuration:transitionContext] animations:^{
        ...
        if(self->_isOnlyShowRangeForRect) {
            bgView.backgroundColor = toView.backgroundColor ? toView.backgroundColor : transit_WhiteTintColor;
        }else{
            bgView.alpha = !isPresenting ? 1.f : 0.f;
        }
        rectView.transform = bgView.transform;
...
//    NSAssert(self.isPushOrPop, @"TLViewTransitionAnimator: 只支持Push/pop转场");
    ...
    CGFloat offsetX = transit_ScreenW - 100.f;
...
- (void)natGeoTypeTransition:(id<UIViewControllerContextTransitioning>)transitionContext presenting:(BOOL)isPresenting {
    ...
    t = CATransform3DTranslate(t, transit_ScreenW * 0.85f, -40.0f, 150.0f);
