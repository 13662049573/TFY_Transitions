...
@interface TFY_PresentationVC ()<UIViewControllerAnimatedTransitioning>{
...
@implementation TFY_PresentationVC
...
+ (BOOL)isIPhoneX {
    ...
    BOOL isIPhoneX = transit_isIPhoneX;
    return isIPhoneX;
}
...
+ (instancetype)showView:(UIView *)popView toPoint:(CGPoint)point {
    return [self showView:popView
                 showType:ShowTypePoint
                  popType:PopTypeAlert
                  toPoint:point
             initialFrame:CGRectNull
               finalFrame:CGRectNull];
}
...
+ (instancetype)showView:(UIView *)popView
    ...
    TFY_PopTransitVC *toVc = [[TFY_PopTransitVC alloc] init];
    [popView layoutIfNeeded];
    toVc.popView = popView;
    
    TFY_PresentationVC *pt;
    pt = [[TFY_PresentationVC alloc] initWithPresentedViewController:toVc
                                      presentingViewController:topVc];
    pt.showType = showType;
    
    if (showType == ShowTypeFrame) {
        pt.initialFrame = iFrame;
        pt.finalFrame = fFrame;
        pt.popView.bounds = CGRectMake(0, 0, fFrame.size.width, fFrame.size.height);
    }else if (showType == ShowTypePoint){
        pt.showPoint = point;
    }
...
- (void)presentationTransitionWillBegin
{
    UIView *presentedViewControllerView = [super presentedView];
    {
        ...
        if (_hideShadowLayer == NO) {
            ...
            CGSize size = self.pType == PopTypeActionSheet ? CGSizeMake(0, -6.f) : CGSizeMake(6, 6.f);
         ...
         */
        // 圆角
        CGFloat H = self.pType == PopTypeActionSheet ? -self.cornerRadius : 0.f;
...
- (CGRect)frameOfPresentedViewInContainerView
{
    ...
    if(_showType == ShowTypePoint){
        ...
        if (transit_isIPhoneX) { // 越界
            ...
            CGFloat H = isLandscape ? transit_ScreenH - transit_iPhoneXHomeBarH : transit_ScreenH - transit_iPhoneXHomeBarH - transit_StatusBarH;
            size.width = size.width > W ? W : size.width;
            size.height = size.height > H ? H : size.height;
        }else {
            size.width = size.width > transit_ScreenW ? transit_ScreenW : size.width; // 越界
            size.height = size.height > transit_ScreenH ? transit_ScreenH : size.height;
        }
        ...
        x = x + size.width > transit_ScreenW ? transit_ScreenW - size.width : x;
        if (transit_isIPhoneX) {
            CGFloat H = transit_ScreenH - transit_iPhoneXHomeBarH;
            y = y + size.height > H ? H - size.height : y;
        }else {
            y = y + size.height > transit_ScreenH ? transit_ScreenH - size.height : y;
        }
        ...
        }else if(self.pType == PopTypeAlert || self.pType == PopTypeAlert2){
            // 垂直居中
                                  ...
                                  toView:(UIView *)toView
{
    ...
    }else if(self.pType == PopTypeAlert2){
        [containerView addSubview:toView];
    ...
    }else if (self.pType == PopTypeActionSheet){
        BOOL isPresenting = (fromViewController == self.presentingViewController);
